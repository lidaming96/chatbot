...
class DoubaoClient:
    ...
    def analyze_image(self, image_data_url, prompt, model="doubao-seed-1-6-vision-250815"):
        ...
        try:
            ...
            error_msg = str(e)
            logger.error(f"Doubao图片分析失败: {error_msg}")
            # 输出更详细的错误信息用于调试
            if hasattr(e, 'response') and hasattr(e.response, 'text'):
                logger.error(f"API响应: {e.response.text}")
            st.error(f"Doubao图片分析失败: {error_msg}")
            return None
...
def analyze_image_with_vision(image_data_url, existing_events=[], existing_profile=[], filename="图片", provider="doubao"):
    ...
    prompt = get_profile_extraction_prompt(
        ...
        include_title_description=True  # 图片提取需要 title 和 description
    )
    ...
    try:
        ...
        if provider.lower() == "doubao":
            try:
                client = get_db_client()
                result_text = client.analyze_image(image_data_url, prompt, model="doubao-seed-1-6-vision-250815")
                if result_text:
                    logger.info("Doubao图片分析成功")
            except Exception as e:
                last_error = f"Doubao API错误: {str(e)}"
        ...
        if result_text is None:
            try:
                ...
                if result_text:
                    logger.info("DeepSeek图片分析成功")
            except Exception as e:
                if last_error:
                    last_error += f" | DeepSeek API错误: {str(e)}"
                else:
                    last_error = f"DeepSeek API错误: {str(e)}"
                logger.warning(f"DeepSeek图片分析失败: {str(e)}")
        
        # 如果都失败，返回错误信息
        if result_text is None:
            error_description = f"图片已上传（{filename}），但当前API配置不支持图片识别功能。"
            if last_error:
                error_description += f" 错误详情: {last_error}"
            else:
                error_description += " 请检查是否使用了支持多模态的API（如Doubao Vision或DeepSeek Vision）。"
            ...
            return {
                ...
                "events": [],
                "profile": [],
                "structured_profile": {},  # API不支持时返回空的结构化画像
                "error": "API不支持图片识别",
        ...
        try:
            ...
            if structured_profile is None:
                ...
                logger.warning(f"structured_profile 格式不正确，期望字典，实际类型: {type(structured_profile)}")
                structured_profile = {}
            ...
            return {
                ...
                "structured_profile": structured_profile,  # 添加结构化画像
                "image_data_url": image_data_url,
                "raw_response": original_response,  # 保存原始响应用于调试
                "cleaned_response": result_text,  # 保存清理后的响应
                "parsed_data": parsed_data  # 保存解析后的完整数据
            }
            ...
            return {
                ...
                "events": [],
                "profile": [],
                "structured_profile": {},  # JSON解析失败时返回空的结构化画像
                "image_data_url": image_data_url,
                "raw_response": original_response,
                "cleaned_response": result_text,
                "json_error": str(e),  # 保存JSON解析错误
                "parsed_data": None
            }
        ...
        return {
            ...
            "events": [],
            "profile": [],
            "structured_profile": {},  # 处理失败时返回空的结构化画像
